init -10 python:
	...
	def show_pause():
		...
		pause_screen.showed_time = get_game_time()
		pause_screen.hided_time = None
		show_screen('pause')
	pause_screen.show = show_pause
	
	for underlay in config.underlay:
		if 'pause' in underlay:
			underlay['pause'] = show_pause
	...
	def get_pause_alpha():
		if pause_screen.hided_time is not None:
